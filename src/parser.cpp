...
{
    term_counter(function); //calc of term crowd
    terms = new std::string[total_terms]; //allocating space (string) for each term 
    const char* ptr = &function[0];
    const char* term_ptr = &function[0];
    unsigned int term_length = 0, parenthesis_cnt = 0;
    int index = -1;
    for(; ; ptr++)
    {
        term_length++;
        size_t start_of_term = term_ptr - &function[0]; //(differnce) between a pointer of term's start and the string start
        if(*ptr == '(')
            parenthesis_cnt++;
        else if(*ptr == ')')
            parenthesis_cnt--;
        if(!parenthesis_cnt && (*ptr == '+' || *ptr == '-' || *ptr == '\0')) //checking till '\0' because the last term does not finish with term
        {
            index++;
            terms[index] = function.substr(start_of_term, term_length-1); //(term_length -1) without the operator
            if(*(ptr) == '\0') break;
            term_ptr = ptr + 1; //start of the next term
